function extractProducts() {
    var urls = [
        {
            url: "https://east-digital.myshopify.com/collections/factory-recertified",
            sheetName: "Category - HDD(FR)",
        },
        {
            url: "https://east-digital.myshopify.com/collections/hdd",
            sheetName: "Category - HDD(New)",
        },
        {
            url: "https://east-digital.myshopify.com/collections/pulls-hdd",
            sheetName: "Category - HDD(Pulls)",
        },
    ];
...
function processUrl(baseUrl, sheetName) {
    ...
    if (!sheet) {
        ...
        if (
            firstRow[0] !== "Product Name" ||
            firstRow[1] !== "Regular Price (NZD)" ||
            firstRow[2] !== "Sale Price (NZD)" ||
            firstRow[3] !== "Date" ||
            firstRow[4] !== "Drive Size (TB)" ||
            firstRow[5] !== "Price per TB (NZD)" ||
            firstRow[6] !== "Stock Status"
        ) {
    ...
    while (hasMorePages) {
        ...
        var noProductsMessage =
            $("#ProductGridContainer .collection--empty .title.title--primary")
                .length > 0;
        ...
        if (products.length === 0 || noProductsMessage) {
            ...
            products.each(function () {
                ...
                if (productUrl) {
                    productUrl =
                        "https://east-digital.myshopify.com" + productUrl;
                }
                ...
                if (
                    priceWrapper.find('.badge:contains("Sold out")').length >
                        0 ||
                ...
                if (productName && productUrl && regularPrice) {
                    ...
                    var driveSize = driveSizeMatch
                        ? parseInt(driveSizeMatch[1])
                        : "";
